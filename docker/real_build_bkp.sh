...
#
# Usage:
#   ./docker/real_build.sh              # build all real robot images
#   ./docker/real_build.sh robot        # build only robot drivers image
#   ./docker/real_build.sh slam         # build only slam image
#   ./docker/real_build.sh robot slam   # build multiple
# =============================================================================
        ...
        case "$target" in
            robot)
                build_image "robot" \
                    ...
                    "${SCRIPT_DIR}/Dockerfile.real"
                ;;
            slam)
                build_image "slam" \
                    "${REPO}:fastbot-ros2-slam-real" \
                    "${SCRIPT_DIR}/Dockerfile.slam-real"
                ;;
        esac
    done
